      ...
      <div className="p-6 max-w-7xl mx-auto space-y-6">
        {isLoading ? (
          ...
          Object.entries(byBranch).map(([branch, branchCases]) => {
            const expanded = expandedBranches.has(branch)
            const visibleCases = expanded ? branchCases : branchCases.slice(0, 5)
            return (
            <div key={branch} className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
              ...
              <div className="divide-y divide-slate-100 dark:divide-slate-800">
                ...
                {branchCases.length > 5 && (
                  <button
                    onClick={() => setExpandedBranches((current) => {
                      const next = new Set(current)
                      if (next.has(branch)) next.delete(branch)
                      else next.add(branch)
                      return next
                    })}
                    className="w-full px-5 py-2 text-xs text-blue-600 font-medium text-center hover:bg-blue-50 dark:hover:bg-blue-950/30"
                  >
                    {expanded ? 'Show fewer cases' : `+${branchCases.length - 5} more cases`}
                  </button>
                )}
              </div>
            </div>

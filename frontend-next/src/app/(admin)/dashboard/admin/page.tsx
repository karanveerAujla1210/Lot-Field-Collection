...
import { useCollectionTrend } from '@/hooks/usePayments'
...
export default function AdminDashboardPage() {
  ...
  const { data: collectionTrend = [], isLoading: collectionTrendLoading } = useCollectionTrend()
    ...
    <div className="min-h-screen">
      <header className="sticky top-0 h-16 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center px-6 z-40">
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={() => toast.info('Notifications are shown in the follow-up queue.')} aria-label="View notifications" className="relative text-slate-500 hover:text-blue-600">
            <Bell size={20} />
      ...
      <div className="p-6 max-w-7xl mx-auto space-y-6">
        <div className="flex justify-between items-end">
          ...
          <div className="flex gap-2">
            <button onClick={() => refetch()} className="flex items-center gap-1.5 px-3 py-2 text-xs font-medium bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg hover:bg-slate-50 transition-colors">
              <RefreshCw size={14} /> Refresh
            </button>
            <button onClick={() => window.print()} className="flex items-center gap-1.5 px-3 py-2 text-xs font-medium bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg hover:bg-slate-50 transition-colors">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {casesLoading ? (
            ...
            <>
              ...
              <KpiCard title="Active Executives" value={`${staff.length}`} icon={Users} trend="All online" color="border-purple-500" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-xl p-5 border border-slate-200 dark:border-slate-800 shadow-sm">
            ...
            {collectionTrendLoading ? <Skeleton className="h-48" /> : collectionTrend.length === 0 ? (
              <div className="h-48 flex items-center justify-center text-sm text-slate-400">No payment activity recorded yet</div>
            ) : <ResponsiveContainer width="100%" height={200}>

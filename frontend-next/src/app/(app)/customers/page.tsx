        ...
        <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
          {['ALL', 'LOW', 'MEDIUM', 'HIGH', 'CRITICAL'].map(r => (
            <button key={r} onClick={() => setFilterRisk(r)}
              className={`px-3 py-1 rounded-full text-xs font-semibold whitespace-nowrap transition-colors ${filterRisk === r ? 'bg-blue-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400'}`}>
              {r}
            </button>
          ))}
        </div>
      ...
      <div className="px-4 py-4 space-y-3">
        {isLoading
          ...
          : filtered.map((c: CaseRow) => {
            ...
            return (
              <div key={c.id} className="bg-white dark:bg-slate-900 rounded-xl p-4 border border-slate-100 dark:border-slate-800 hover:shadow-sm transition-all">
